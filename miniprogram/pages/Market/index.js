Page({
  ...
  filterItem(){
    ...
    this.setData({
      unboughtItems: itemList,
      boughtItems: [],  // 已购买商品可以不再显示，设置为空数组
    })
    ...
    await wx.cloud.callFunction({name: 'getOpenId'}).then(async openid => {
        ...
        if (index === 0) {
            ...
            //处理删除按钮点击事件
                ...
                await wx.cloud.callFunction({name: 'addNews', data: {desc:"下架了物品【"+item.title+"】，以后买不到咯！"}}).then()
            }
  ...
  sendSubscribeMessage(item) {
    ...
    wx.cloud.callFunction({
      ...
      data: {
          ...
          taskName: '叮咚～有人买你的东西啦！',
          desc: "购买物品【"+item.title+"】",
          credit: "TA消费了 "+item.credit + " 积分",
  ...
  async buyItem(element) {
    ...
    await wx.cloud.callFunction({name: 'getOpenId'}).then(async openid => {
      ...
      //如果没有积分，显示提醒
        ...
        await wx.cloud.callFunction({name: 'addNews', data: {desc:"购买了物品【"+item.title+"】, 花掉了" + item.credit + "积分！"}}).then()

        // item.available = false
        // this.filterItem()
      }
    })
    await this.sendSubscribeMessage(item);
  },
})