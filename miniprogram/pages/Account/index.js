      ...
      await wx.cloud.callFunction({name: 'getOpenId'}).then(async openid => {
          ...
          if (index === 0) {
              ...
              //处理删除按钮点击事件
                  ...
                  await wx.cloud.callFunction({name: 'addNews', data: {desc:"丢弃了物品【"+item.title+"】， 真浪费呀~"}}).then()
              }
  ...
  sendSubscribeMessage(item) {
    ...
    wx.cloud.callFunction({
      ...
      data: {
          ...
          taskName: '叮咚～有人使用道具了，快去兑现吧！',
          desc: "使用物品【"+item.title+"】",
          credit: "这个物品价值 "+item.credit + " 积分",
    ...
    async useItem(element) {
        ...
        await wx.cloud.callFunction({name: 'addNews', data: {desc:"使用了物品【"+item.title+"】！"}}).then()
